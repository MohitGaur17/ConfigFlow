...
export default function RegisterPage() {
  ...
  return (
    <>
      <div className="glass-card w-full border border-outline-hairline rounded-xl p-8 flex flex-col shadow-2xl relative overflow-hidden animate-in fade-in zoom-in-95 duration-300">
        ...
        <form onSubmit={handleRegister} className="flex flex-col gap-4 mb-8">
          <div className="flex flex-col gap-1.5">
            <label className="font-label-tech text-label-tech text-on-surface-variant ml-1">FULL NAME</label>
            <input 
              value={name}
              onChange={(e) => setName(e.target.value)}
              className="input-field w-full bg-surface-dim/50 text-on-surface border-0 border-b border-outline-variant px-0 py-3 focus:ring-0 focus:border-primary placeholder-on-surface-variant font-code-base text-code-base transition-colors bg-transparent" 
          ...
          <div className="pt-4">
            <button 
              type="submit" 
              disabled={isLoading}
              className="glow-button w-full bg-primary-container text-white font-headline-md text-[16px] py-3 px-4 rounded font-bold tracking-wide hover:bg-orange-600 transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
            >
